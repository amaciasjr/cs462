ruleset wovyn_base_lab8 {
  ...
  global {
    ...
    location = function() {
      ent:sensor_loc.defaultsTo(temp_location)
    };
    
    name = function() {
      ent:sensor_name.defaultsTo(temp_name)
    };
  ...
  rule profile_updated {
    ...
    pre {
      ...
      passed_location = event:attr("location") => event:attr("location") | location()
      passed_name = event:attr("name") => event:attr("name") | name()
